"""
ObjectiveMoneySub - Take player money objective.
"""
...
class ObjectiveMoneySub(ObjectiveBase):
    """
    Money Subtract objective - Take money from the player.

    Removes a specified amount of money from the player's cash. Completes immediately.

    Note: This objective does not use interior ID or position fields meaningfully.

    Default text: "" (no text, instant money subtraction)
    """

    # Objective type
    objective_type: Literal[21] = Field(21, description="Objective type (21 = Money Subtract)")

    # Money amount
    money: int = Field(
        100,
        description="Amount of money to take from player",
        ge=0
    )
